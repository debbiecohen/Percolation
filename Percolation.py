...
class PercolationQF(object):
        ...
        for i in range(1, self.size+1):
            ...
            for j in range(1, self.size+1):
                if self.is_full(i, j):
                    ...
                    row.append(2)                                               #2 are going to be the white sites
                else:
    ...
    def draw(self, canvasSize):
        for i in range(self.size):
            for j in range(self.size):
                if self.is_full(i+1,j+1):
                    Draw.setColor(Draw.CYAN)
                    Draw.filledRect(j*20, i*20, 20,20)               
        Draw.setColor(Draw.BLACK)
        for i in range(0, canvasSize, 20):
            Draw.line(i,0,i,canvasSize-1)
            Draw.line(0,i,canvasSize-1,i)  
            
    def animate(self):
        canvasSize: int = self.size*20 +1
        ...
        while not self.percolates():               
            randX = random.randint(1,self.size)
            randY = random.randint(1,self.size)
            if not self.is_open(randX, randY):
                self.open(randX, randY) 
                Draw.setColor(Draw.RED)
                Draw.filledRect((randY-1)*20, (randX-1)*20, 20,20) 
                Draw.setColor(Draw.WHITE)
                time.sleep(.3)
                Draw.filledRect((randY-1)*20, (randX-1)*20, 20,20)            
        self.draw(canvasSize)
